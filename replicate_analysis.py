...
def plot_fitnesses_sep(fitness_logs, eval_func_names, save_loc, transparent=False):
    ...
    for func_name in eval_func_names:
        ...
        axis[sp].plot(eval_func_data_mean, color="#509154")
...
def plot_spreads(diversity_logs, property_names, save_loc, logscale=False, transparent=False):
    figure, axis = plt.subplots(1, 1)
    ...
    for func_name in property_names:
        ...
        axis.plot(data_mean, label=func_name)
        axis.fill_between(range(len(data_mean)), neg_error, pos_error, alpha=0.5)
    if logscale:
        axis.set_yscale("log")
    figure.supxlabel("Generations")
